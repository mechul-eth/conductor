...
MG_MODE is a markdown-first orchestration layer that sits on top of three proven open-source libraries and adds deterministic routing, cost control, governance gates, and cross-role session state. It works inside your IDE — no server, no compiled binary, no runtime dependencies beyond your AI coding agent.

You describe what you want. MG_MODE figures out which specialist(s) should do it, how much it should cost, whether it needs a governance gate, and how to hand context from one role to the next — then watches the work, catches scope drift, and persists state so nothing gets lost between sessions.

---

## Standing on the Shoulders of Giants

MG_MODE exists because three open-source projects already solved the hard problems. We didn't rebuild what they built — we built the routing layer that connects them.

### [The Agency](https://github.com/msitarzewski/agency-agents) — by Mike Sitarzewski

156 meticulously crafted AI agent role definitions across 13 domains — engineering, design, marketing, product, testing, sales, project management, support, paid media, spatial computing, game development, academic, and strategy. Each agent has a defined personality, communication style, core workflows, technical deliverables, and success metrics. Born from a Reddit thread and months of iteration, The Agency provides the talent pool that MG_MODE routes to.

**What MG_MODE uses it for:** Every specialist role in the system — from Backend Architect to Whimsy Injector — is a real file from this library. The registry indexes all 156 roles with capability fingerprints so the routing algorithm can find the right agent in milliseconds. MG_MODE never invents roles; it only activates what exists here.
...
**What MG_MODE uses it for:** Workflow execution patterns. When the conductor routes a task to "review mode," it's gstack's `/review` skill that structures the code review. When QA is needed, gstack's `/qa` skill opens a real browser. MG_MODE reads these patterns but never modifies them — `proactive=false` is enforced at activation to prevent gstack's scope-expansion behavior from overriding MG_MODE's routing policy.
...
└── mg-mode-core/       ← Layer 2: The orchestration brain (this is what MG_MODE adds)
    ├── MG_MODE.md      ← Master policy — routing rules, session lifecycle, action classification
...
**Layer 1** is the talent and tooling — three mature open-source projects used as-is, read-only. MG_MODE never modifies their files.
...
# Start working — MG_MODE handles routing from here
```
...
| Component | Purpose |
|-----------|---------|
| [MG_MODE.md](mg-mode-core/MG_MODE.md) | Master policy — routing algorithm, session lifecycle, action classification, loop safety, bypass prevention |
...
MG_MODE's orchestration layer (mg-mode-core/) is [MIT licensed](LICENSE).
...
MG_MODE wouldn't exist without the work of [Mike Sitarzewski](https://github.com/msitarzewski), [Garry Tan](https://github.com/garrytan), and the [Promptfoo](https://github.com/promptfoo) team. Their open-source projects are the foundation this orchestrator stands on. We built the brain — they built the body.
